...
export function CLOCK(): string {
  const t = new Date();
  const date = `${t.getDate().toString().padStart(2, '0')}/${(t.getMonth() + 1)
    .toString()
    .padStart(2, '0')}/${t.getFullYear()}`;
  const time = `${t.getHours().toString().padStart(2, '0')}:${t
    .getMinutes()
    .toString()
...
// ─── CORE LOGGER ─────────────────────────────────────
function _log(severity: Severity, source: string, message: string | Error, event: string): void {
...
export const Log = {
  info:
     ...
     * @returns
     */
    (source: string, message: string, event: string) => _log('INFO', source, message, event),
  ...
  trace:
     ...
     * @returns
     */
    (source: string, message: string, event: string) => _log('TRACE', source, message, event),
  ...
  guide:
     ...
     * @returns
     */
    (source: string, message: string, event: string) => _log('GUIDE', source, message, event),
