...
// to release. ──
async function USER_PROFILE(payload: {
  user_id: string;
  org_id?: string;
...
const ORG_ROLE_MAP: Record<'yellow' | 'green', Record<ORG_ROLE, ROLES>> = {
  ...
  green: {
    voter: 'VOTER_ACCESS_PASS[PART]',
    admin: 'ORG_ACCESS[PART]',
    moderator: 'ORG_ACCESS[FULL]',
  },
};
...
// ── PDP entry point ──
async function Operations_Manager(payload: {
  user_id: string;
  org_id?: string;
  location: LOCATION;
}) {
