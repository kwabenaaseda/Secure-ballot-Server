...
export async function GenerateToken({
  ...
  network,
  data,
}: auth_generate_token_payload): Promise<Service_Error_Handler | string | boolean> {
  ...
  try {
    let EXPIRY = '5m';
    if (
      range == 'ACCOUNT_ACCESS[FULL]' ||
      range == 'ORG_ACCESS[FULL]' ||
      range == 'SYSTEM_ADMIN' ||
      range == 'NO_ACCESS' ||
      range == 'SELF_ACCOUNT_ACCESS' ||
      range == 'ORG_ACCESS[PART]'
    ) {
      EXPIRY = ACCESS_TOKEN_EXPIRY;
    } else if (
      range == 'ACCOUNT_ACCESS[PART]' ||
      range == 'VOTER_ACCESS_PASS[FULL]' ||
      range == 'VOTER_ACCESS_PASS[PART]'
    ) {
      EXPIRY = '8m';
    }
    ...
    var token = jwt.sign(payload, ENV('JWT_SECRET') as jwt.Secret, options);
    return token;
  } catch (error) {
...
export async function VerifyToken(token: string) {
  return jwt.verify(token, ENV('JWT_SECRET') as jwt.Secret, { ignoreExpiration: false });
}
