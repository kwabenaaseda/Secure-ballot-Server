...
const VONAGE_MESSAGES_API_HOST =
  ENV('VONAGE_MESSAGES_API_HOST') || 'https://messages-sandbox.nexmo.com';
...
export async function sendWhatsApp(payload: WhatsAppPayload): Promise<boolean> {
  ...
  if (!whatsappClient) {
    console.warn(
      '⚠️  WhatsApp not configured (VONAGE_WHATSAPP_ENABLED is false or app id missing).'
    );
