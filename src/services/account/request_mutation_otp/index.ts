...
export async function RequestMutationOTP_Operation(params: {
  userId: string;
  network: NetworkContext;
}) {
  const started_at = Date.now();
  const ops_base = {
    event: 'REQUEST_MUTATION_OTP',
    source: 'RequestMutationOTP_Operation',
  ...
  await sendOTPEmail({ to: user.email, username: user.username, otp }).catch((err) =>
    console.error('Mutation OTP email failed:', err)
  );

  return await OPS_Success({
    ...ops_base,
    status: 'COMPLETED',
    message: 'Verification code sent. Confirm to proceed with account changes.',
  });
}
