...
export async function VerifyMutationOTP_Operation(params: {
  userId: string;
  otp: string;
  ...
  const ops_base = {
    event: 'VERIFY_MUTATION_OTP',
    source: 'VerifyMutationOTP_Operation',
  ...
  const otp_check = await VerifyOTP_Operation({
    userId: params.userId,
    otp: params.otp,
    network: params.network,
    caller: 'VerifyMutationOTP_Operation',
  });
  ...
  return await OPS_Success({
    ...ops_base,
    status: 'COMPLETED',
    message: 'Verified. You may now update or delete your account.',
    data: { token },
  });
}
