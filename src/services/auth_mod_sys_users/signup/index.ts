...
export async function Signup_Operation(
  ...
  try {
    ...
    const { username, email, telephone, password, date_of_birth, nationality_code, occupation } =
      payload;
    ...
    sendWelcomeEmail({ to: savedUser.email, username: savedUser.username }).catch((err) =>
      Log.debug(SOURCE, `Welcome email failed: ${err}`, EVENT)
    );

    sendOTPEmail({ to: savedUser.email, username: savedUser.username, otp }).catch((err) =>
