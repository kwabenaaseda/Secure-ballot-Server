...
export async function ApproveOrganization_Operation(
  payload: { admin_id: string; network: NetworkContext; org_id: string }
  ...
  try {
    ...
    const org = await orgRepo.findOne({ where: { id: payload.org_id }, relations: ["primary_admin"] });

    if (!org) {
      await queryRunner.rollbackTransaction();
      return await OPS_Error({
        ...ops_base, status: "OPERATION_FAILURE", message: "Organization not found.",
        error_code: "ORG_NOT_FOUND", error_category: "VALIDATION", retryable: false,
      });
    }
    if (org.status !== "pending") {
      await queryRunner.rollbackTransaction();
      return await OPS_Error({
        ...ops_base, status: "OPERATION_FAILURE",
        message: `Organization is already ${org.status}, not pending.`,
        error_code: "INVALID_ORG_STATE", error_category: "VALIDATION", retryable: false,
      });
    }
    ...
    return await OPS_Success({
      ...ops_base, status: "COMPLETED", message: "Organization approved.",
    ...
    return await OPS_Error({
      ...ops_base, status: "SYSTEM_FAILURE", message: "Failed to approve organization.",
...
export async function RejectOrganization_Operation(
  payload: { admin_id: string; network: NetworkContext; org_id: string; reason: string }
): Promise<Service_Success_Handler | Service_Error_Handler> {
  const ops_base = base("ADMIN_REJECT_ORG", payload.admin_id, payload.network, payload.org_id);
  try {
    if (!payload.reason) {
      return await OPS_Error({
        ...ops_base, status: "OPERATION_FAILURE", message: "A rejection reason is required.",
        error_code: "MISSING_REQUIRED_FIELDS", error_category: "VALIDATION", retryable: true,
      });
    }
    ...
    return await OPS_Success({
      ...ops_base, status: "COMPLETED", message: "Organization rejected.",
      data: { org_id: org.id, status: org.status },
    });
  } catch (error) {
    Log.debug(SOURCE, String(error), "ADMIN_REJECT_ORG");
    return await OPS_Error({
      ...ops_base, status: "SYSTEM_FAILURE", message: "Failed to reject organization.",
      error_code: "INTERNAL_ERROR", error_category: "SYSTEM", retryable: true, retry_after_ms: 5000,
...
export async function SuspendOrganization_Operation(
  payload: { admin_id: string; network: NetworkContext; org_id: string; reason?: string }
): Promise<Service_Success_Handler | Service_Error_Handler> {
  ...
  try {
    ...
    return await OPS_Success({
      ...ops_base, status: "COMPLETED", message: "Organization suspended. All member access revoked.",
      data: { org_id: org.id, status: org.status },
    });
  } catch (error) {
    await queryRunner.rollbackTransaction();
    ...
    return await OPS_Error({
      ...ops_base, status: "SYSTEM_FAILURE", message: "Failed to suspend organization.",
      error_code: "INTERNAL_ERROR", error_category: "SYSTEM", retryable: true, retry_after_ms: 5000,
