...
export async function OPS_Success(params: OPS_Success_Params): Promise<Service_Success_Handler> {
...
export async function OPS_Error(params: OPS_Error_Params): Promise<Service_Error_Handler> {
  const meta = buildMeta(params, params.started_at);
  const security = buildSecurityAudit(params);
