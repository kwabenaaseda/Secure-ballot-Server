...
const GENESIS_HASH = createHash('sha256').update('SECUREVOTE_AUDIT_CHAIN_GENESIS_V1').digest('hex');
...
async function getChainTailInTransaction(
  ...
  await queryRunner.query(`SELECT pg_advisory_xact_lock($1)`, [AUDIT_CHAIN_LOCK_KEY]);

  // Now we are the only writer. Safe to read the tail.
  const rows: Array<{ chain_hash: string; sequence_number: string }> = await queryRunner.query(
...
export async function buildIntegrityProof(
  ...
  const entry_hash = createHash('sha256').update(canonicalJSON(entry_content)).digest('hex');
  ...
  const signature = createHmac('sha256', SERVER_SIGNING_KEY).update(entry_hash).digest('hex');
