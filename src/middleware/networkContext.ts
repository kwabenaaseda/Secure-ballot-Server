...
export async function NetworkContextMiddleware(req: Request, res: Response, next: NextFunction) {
  Log.info(
    NetworkContextMiddleware.name,
    'Attempting Network Context extraction',
    'SYSTEM_NETWORK_CONTEXT_EXTRACTION'
  );
  try {
    // Session ID
    ...
    if (!_ip) {
      Log.warn(
        NetworkContextMiddleware.name,
        'Possible Infiltration detected',
        'NO_IP_FROM_SOURCE'
      );
