...
const MATRIX: { role: string; resource: string; action: string; effect?: 'ALLOW' | 'DENY' }[] = [
  ...
  {
    role: 'VOTER_ACCESS_PASS[PART]',
    resource: 'election.result_certified',
    action: 'read',
    effect: 'ALLOW',
  },
  {
    role: 'VOTER_ACCESS_PASS[PART]',
    resource: 'election.tally_live',
    action: 'read',
    effect: 'DENY',
  },
  ...
  {
    role: 'ORG_ACCESS[PART]',
    resource: 'org_membership',
    action: 'verify_join_request',
    effect: 'ALLOW',
  },
...
async function seed() {
  ...
  for (const entry of MATRIX) {
    await repo.upsert({ ...entry, effect: entry.effect ?? 'ALLOW' }, [
      'role',
      'resource',
      'action',
    ]);
